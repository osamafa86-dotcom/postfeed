...
<style>
  ...
  .net-hero {
    background:linear-gradient(135deg,#2C2416 0%, #1E1F18 100%);
  ...
  .net-hero::before {
    ...
    background:
      radial-gradient(circle at 12% 20%, rgba(61,90,40,.28), transparent 55%),
      radial-gradient(circle at 88% 80%, rgba(245,158,11,.20), transparent 55%);
    pointer-events:none;
  }
  .net-eyebrow {
    display:inline-flex; align-items:center; gap:8px;
    background:rgba(61,90,40,.20); color:#A6C098;
    border:1px solid rgba(61,90,40,.45); padding:6px 14px;
    border-radius:999px; font-size:12px; font-weight:800;
    margin-bottom:14px; position:relative;
  }
  .net-title { font-size:30px; font-weight:900; line-height:1.4; margin-bottom:10px; position:relative; }
  .net-lede { font-size:14.5px; line-height:1.85; color:#C5BFAB; max-width:780px; position:relative; }
  .net-stats {
    display:flex; flex-wrap:wrap; gap:18px; margin-top:18px;
    font-size:13px; color:#968B78; font-weight:600; position:relative;
  }
  ...
  .net-stage .link {
    stroke:#968B78; stroke-opacity:.45; fill:none;
    transition:stroke .2s ease, stroke-opacity .2s ease;
  }
  .net-stage .link.highlight { stroke:#3D5A28; stroke-opacity:.95; }
  ...
  .net-tip {
    ...
    background:#2C2416; color:#fff; padding:10px 14px;
...
<div class="net-container">
  ...
  <?php else: ?>
    ...
    <div class="net-legend">
      <span>🔍 اقرأ هكذا:</span>
      <span class="chip"><span class="dot" style="background:#3D5A28;"></span> حجم الدائرة ∝ عدد التقارير</span>
