...
<!-- SECTIONS NAV (homepage anchors) — Figma-trimmed set: Home, Daily
     ...
     for direct anchor links. Moved up here (immediately under the
     main nav) so the pills sit beside the global header strip,
     matching where the user wants the section navigator visually. -->
...
<div class="main-layout">
  ...
  <aside class="nfr-aside" aria-label="الشريط الجانبي">
    ...
    //
    // Cached in PHP memory for the rest of this request only — the
    // input data is itself cached upstream, so no extra DB hits.
    $cleanTrends = [];
    ...
    if (count($cleanTrends) < 3) {
        ...
        // 2a — categories that actually have fresh content
        $pool = array_merge(
        ...
        // 2b — multi-source clusters from the same pool (strong
        // signal: the same story showing up across outlets right now)
        if (count($cleanTrends) < 6 && !empty($GLOBALS['__nf_cluster_counts'])) {
            $clusterTitles = [];
            foreach ($pool as $a) {
                $k = (string)($a['cluster_key'] ?? '');
                if ($k === '' || $k === '-') continue;
                $cnt = (int)($GLOBALS['__nf_cluster_counts'][$k] ?? 0);
                if ($cnt < 2) continue;
                $title = trim((string)($a['title'] ?? ''));
                if ($title === '' || isset($seenTitles[$title])) continue;
                // shorten so we still look like a tag, not a sentence
